// fragment.ts - `use:fragment={{text}}` renders AUTHORED MARKDOWN into an
...
export interface FragmentSpec {
  /** raw Markdown, exactly as the author wrote it */
  text: string;
  /**
   * true for a single-line context (a requirement description): inline
   * constructs only, no wrapping <p>. Block by default, because a test step may
   * be a list or a code block.
   */
  inline?: boolean;
}

/**
 * @param node - the cell to fill; assumed to have no other children
 */
export function fragment(
  node: HTMLElement,
  spec: FragmentSpec,
): { update: (spec: FragmentSpec) => void; destroy: () => void } {
  /**
   ...
   */
  let shown: FragmentSpec | null = null;

  function render(next: FragmentSpec): void {
  ...
  return {
    ...
    // unmounted while its target stays (islands/requirements.ts unmounts into a
