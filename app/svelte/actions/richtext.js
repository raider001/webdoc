// richtext.js - the two halves of a recorded test response: `use:richtext`
...
//
// WHY THE IMPORT IS DYNAMIC, exactly as in ./graph.js: editor.js is the head of
...
// ./fragment.js's `use:fragment`: fragment renders authored MARKDOWN, this
...
/**
 * @typedef {Object} RichTextSpec
 * @property {string} value - the field's INITIAL html; never re-applied
 * @property {(html: string) => void} onchange - fires on every input event
 * @property {string} [placeholder] - shown by CSS while the field is empty
 * @property {string} [extraClass] - added to the field itself, not to the host,
 *   so a rule written against `.run-notes` still lands on the editable box
 */

/**
 * @param {HTMLElement} node - the host cell; the field is APPENDED, so any text
 *   Svelte rendered into it stays in front of the field
 * @param {RichTextSpec} spec
 * @returns {{destroy: () => void}}
 */
export function richtext(node, spec) {
 ...
 * @param {HTMLElement} node - the host; content is APPENDED after whatever
 *   Svelte already put there (the "Actual: " label)
 * @param {string} html - as recorded by a previous run
 * @returns {{destroy: () => void}}
 */
export function sanitized(node, html) {
