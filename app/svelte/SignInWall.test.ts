...
// islands/auth.ts calls startAuthSync() before mounting the wall, and the wall
...
function mountWall(props: ComponentProps<typeof SignInWall>) {
  startAuthSync();
...
const wallOf = (container: HTMLElement): Element => container.querySelector('.auth-wall')!;
...
describe('SignInWall', () => {
  ...
  it('opens on the register form during bootstrap, and offers no way back', () => {
    ...
    expect(container.querySelector<HTMLElement>('.auth-switch')!.hidden).toBe(true);
  ...
  it('hides the display-name field until it is registering, and quotes the real minimum length', async () => {
    const { container } = mountWall({ onSignedIn: () => {} });
    const nameField = container.querySelectorAll<HTMLElement>('.modal-field')[2];
