// shell.svelte.ts - state for the three satellites of the reading view: the
...
export interface ShellState {
  /** the routed document; the breadcrumb is its id, segment by segment */
  docId: string;
  /** the flat heading list numberHeadings() returned, in document order */
  toc: TocEntry[];
  /** doc ids for the "Assumed knowledge" footer group */
  assumes: string[];
  /** doc ids for the "Recommended next" footer group */
  next: string[];
}

export const shellState: ShellState = $state({ docId: '', toc: [], assumes: [], next: [] });

/**
 * The routed document, for the breadcrumb.
 */
export function setCrumbs(docId: string): void { shellState.docId = docId; }
 ...
 */
export function setToc(toc: TocEntry[]): void { shellState.toc = toc; }
 ...
 */
export function setFootLinks(assumes: string[], next: string[]): void {
