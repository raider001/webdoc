...
// in app/js/auth.js, which is unchanged, and stores/auth.svelte.ts only MIRRORS
...
// that is rebuilt on every navigation. See app/svelte/islands/requirements.ts.
...
// a canvas rAF loop and window.__graph alive. See app/svelte/islands/coverage.ts.
...
// is not exported) for two reasons. It is what puts stores/coverage.svelte.ts
...
// gap. islands/coverage.ts will be its ordinary caller once it exists.
