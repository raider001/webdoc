...
import { setupGraphButton, invalidateGraphModel, buildDocGraph } from './map-view.js';
...
async function onExternalChange() {
  ...
  if (document.body.classList.contains('is-editing')) {
    el('live').textContent = 'Files changed on disk. Close the editor to see the latest version.';
  ...
  if (overlay && !overlay.hidden) buildDocGraph(true, true);
}
