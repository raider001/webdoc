...
function trimUrlPunct(url) {
    ...
    while (changed && url.length && ++guard < 200) {
        ...
        if (lastCh === ';') {
            ...
            if (em) {
                trail = em[0] + trail;
                url = url.slice(0, url.length - em[0].length);
                changed = true;
                continue;
            }
        }
...
function emitTextRun(pieces, run, prevChar) {
    ...
    while ((m = RE_BAREURL.exec(run)) !== null) {
        ...
        if (!link)
            continue; // leave as plain text; scanning resumes past this match
        if (start > last)
            pieces.push({ kind: 'text', text: esc(run.slice(last, start)) });
    ...
    if (last < run.length)
        pieces.push({ kind: 'text', text: esc(run.slice(last)) });
}
/**
    ...
    while (i < n) {
        ...
        if (c === '\n') {
            ...
            if (prev && prev.kind === 'text') {
                ...
                if (/  $/.test(t)) {
                    prev.text = t.replace(/ +$/, '');
                    pieces.push({ kind: 'hardbreak' });
                }
                else if (/\\$/.test(t)) {
                    prev.text = t.slice(0, -1);
                    pieces.push({ kind: 'hardbreak' });
                }
                else {
                    prev.text = t.replace(/ +$/, '');
                    pieces.push({ kind: 'softbreak' });
                }
            }
            else
                pieces.push({ kind: 'softbreak' });
        ...
        if (m) {
            emitTextRun(pieces, m[0], i === 0 ? '' : s[i - 1]);
            i += m[0].length;
            continue;
        }
        if (c === '\\') {
            if (i + 1 < n && ESCAPABLE.indexOf(s[i + 1]) !== -1) {
                pieces.push({ kind: 'text', text: esc(s[i + 1]) });
                i += 2;
                continue;
            }
            if (i + 1 < n && s[i + 1] === '\n') {
        ...
        if (c === '&') {
            ...
            if (em) {
                const d = decodeEntity(em[0]);
                if (d !== null) {
                    pieces.push({ kind: 'text', text: esc(d) });
                    i += em[0].length;
                    continue;
                }
            }
        ...
        if (c === '<') {
            ...
            if (mm) {
                const url = mm[1];
                pieces.push({ kind: 'raw', html: '<a href="' + esc(normalizeUri(url)) + '">' + esc(url) + '</a>' });
                i += mm[0].length;
                continue;
            }
            ...
            if (mm) {
                const addr = mm[1];
                pieces.push({ kind: 'raw', html: '<a href="mailto:' + esc(addr) + '">' + esc(addr) + '</a>' });
                i += mm[0].length;
                continue;
            }
            mm = RE_HTMLTAG.exec(s.slice(i));
            if (mm) {
                pieces.push({ kind: 'raw', html: mm[0] });
                i += mm[0].length;
                continue;
            }
            pushText('&lt;');
...
function resolveEmphasis(pieces, delims, _bottom) {
    ...
    while (closerPos < stack.length) {
        if (++guard > stack.length * stack.length + 200)
            break;
        const cIdx = stack[closerPos];
        const closer = pieces[cIdx];
        if (!closer || !closer.delim || closer.used || !closer.canClose || closer.numDelims === 0) {
            closerPos++;
            continue;
        }
        ...
        for (let k = closerPos - 1; k >= 0; k--) {
            ...
            if (!opener || !opener.delim || opener.used || opener.delim !== ch || !opener.canOpen || opener.numDelims === 0)
                continue;
        ...
        opener.numDelims = openerN - use;
        closer.numDelims = closerN - use;
        ...
        for (let k = openerPos + 1; k < closerPos; k++) {
            const mid = pieces[stack[k]];
            if (mid && mid.delim)
                mid.used = true;
        }
...
function handleCloseBracket(s, i, pieces, delims, refs) {
    ...
    let dest = '', title = null, matched = false;
    if (s[j] === '(') {
        let k = j + 1;
        while (k < s.length && /[ \t\n]/.test(s[k]))
            k++;
    ...
    if (!matched) {
        ...
        if (s[i + 1] === '[') {
            ...
            if (lr) {
                refLabel = lr.label.trim() === '' ? rawLabel : lr.label;
                endPos = lr.pos;
            }
            else
                refLabel = null;
    ...
    if (isImage) {
        ...
        // longer open a link.
        for (let k = openerDelimPos - 1; k >= 0; k--) {
            const p = pieces[delims[k]];
            if (p && p.bracket === '[')
                p.inactive = true;
        }
...
function serialize(pieces) {
    ...
    for (const p of pieces) {
        if (p.delim) {
            ...
            if (p.wrapClose)
                for (const t of p.wrapClose)
                    out += '</' + t + '>';
            out += p.text || '';
            if (p.wrapOpen)
                for (const t of p.wrapOpen)
                    out += '<' + t + '>';
            continue;
        }
        if (p.used && p.kind === 'text' && !p.wrapOpen && !p.wrapClose && !p.wrapBefore) {
            out += p.text || '';
            continue;
        }
        if (p.wrapBefore)
