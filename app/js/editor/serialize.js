...
function fencedRanges(body) {
    ...
    for (const line of String(body).split('\n')) {
        ...
        if (!open) {
            ...
            if (o)
                open = { ch: o[1][0], len: o[1].length, from: start };
        }
        else {
    ...
    if (open)
        ranges.push([open.from, String(body).length]);
    return ranges;
}
/**
...
function inlineToMd(node) {
    ...
    node.childNodes.forEach(n => {
        ...
        if (tag === 'strong' || tag === 'b')
            ...
            out += '[' + inner + '](' + mdDest(el.getAttribute('href') || '') + ')';
        ...
        // fell through to `inner` (empty) and inline images were silently lost on save.
            ...
            out += '![' + (el.getAttribute('alt') || '') + '](' + mdDest(el.getAttribute('data-mdsrc') || el.getAttribute('src') || '') + ')';
        else if (tag === 'br')
            out += '  \n';
        else
            out += inner;
...
function blockToMd(b) {
    switch (b.type) {
        ...
        case 'access-start': {
            const spec = {};
            if (Array.isArray(b.read) && b.read.length)
                spec.read = b.read.map(String);
            if (b.label)
                spec.label = String(b.label);
...
export function serializeDoc(meta, blocks) {
    ...
    if (meta.access && typeof meta.access === 'object')
        m.access = meta.access;
    // Unknown keys last, and never allowed to overwrite a known one.
    const extra = meta._extra || {};
...
export function parseDoc(rawBody, docMeta) {
    ...
    let source = String(rawBody).replace(ACCESS_TOKEN_RE, (whole, kind, json, offset) => {
        if (inFence(fences, offset))
            return whole;
        if (String(kind).toLowerCase() === 'end') {
            ...
            if (spec.access && typeof spec.access === 'object')
                spec = spec.access;
            markers.push({
                type: 'access-start',
    ...
    holder.childNodes.forEach(n => {
        ...
        if (/^h[1-6]$/.test(tag))
            blocks.push({ type: 'heading', level: +tag[1], html: stripNums(el) });
        else if (tag === 'p')
            blocks.push({ type: 'paragraph', html: el.innerHTML });
        else if (tag === 'blockquote')
            blocks.push({ type: 'quote', html: el.innerHTML.replace(/<\/?p>/g, '').trim() });
        else if (tag === 'ul' || tag === 'ol')
            blocks.push({ type: 'list', ordered: tag === 'ol', itemsHtml: [...el.children].map(li => li.innerHTML) });
        else if (tag === 'pre') {
            const code = el.querySelector('code');
            blocks.push({ type: 'code', lang: langOf(code), code: (code || el).textContent.replace(/\n$/, '') });
        }
        else if (tag === 'hr')
            blocks.push({ type: 'hr' });
        else if (tag === 'table')
...
export function newBlock(type) {
    if (type === 'list-ordered')
        return { type: 'list', ordered: true, itemsHtml: [''] };
    if (type === 'list')
        return { type: 'list', ordered: false, itemsHtml: [''] };
    if (type === 'heading')
        return { type: 'heading', level: 2, html: '' };
    if (type === 'code')
        return { type: 'code', lang: '', code: '' };
    if (type === 'quote')
        return { type: 'quote', html: '' };
    if (type === 'table')
        return { type: 'table', headers: ['Column 1', 'Column 2'], aligns: ['', ''], rows: [['', '']] };
    if (type === 'image')
        return { type: 'image', src: '', alt: '' };
    if (type === 'hr')
        return { type: 'hr' };
    if (type === 'requirement')
        return { type: 'requirement', group: 'grp', rows: [{ no: '1', description: '', traceTo: '' }] };
    if (type === 'testcase')
        return { type: 'testcase', key: 'test', name: '', verifies: [], steps: [{ action: '', expected: '' }] };
    ...
    if (type === 'access')
        return { type: 'access-start', read: [], label: '' };
    if (type === 'access-end')
        return { type: 'access-end' };
