...
export function testCaseWidget(b, getReqs, comp) {
    ...
    append(box, preview);
    updatePreview();
    append(box, elem('label', 'blk-reqgroup blk-tc-verifies', 'Verifies ', refsField(b.verifies || [], refs => b.verifies = refs, getReqs, 'requirement…')));
    append(box, table);
    function draw() {
        table.textContent = '';
        append(table, elem('div', 'req-row tc-erow req-head', span('Action'), span('Expected response'), span('')), b.steps.map((s, i) => elem('div', 'req-row tc-erow', mdField(s.action, v => s.action = v, 'action (markdown ok)…'), mdField(s.expected, v => s.expected = v, 'expected response (markdown ok)…'), smallBtn(closeIcon(), () => { b.steps.splice(i, 1); if (!b.steps.length)
            b.steps.push({ action: '', expected: '' }); draw(); }))), elem('button', { class: 'blk-req-add', onClick: () => { b.steps.push({ action: '', expected: '' }); draw(); } }, plusIcon(), ' Step'));
    }
    /**
...
function refsField(initial, onChange, getReqs, placeholder) {
    ...
    function drawChips() {
        ...
        refs.forEach((ref, i) => {
            const remove = elem('button', { title: 'Remove', onClick: () => {
                    ...
                    drawChips();
                    sync();
                    input.placeholder = refs.length ? '' : (placeholder || '');
                } }, closeIcon());
    ...
    input.addEventListener('keydown', e => {
        if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            addRef(acActive >= 0 && acItems[acActive] ? acItems[acActive].id : input.value);
            closeAc();
        }
        else if (e.key === ',') {
            e.preventDefault();
...
export function accessMarker(b, knownGroups) {
    if (b.type === 'access-end') {
        return elem('div', 'access-marker is-end', lockIcon(), elem('span', 'access-marker-label', 'End of restricted section'));
    }
