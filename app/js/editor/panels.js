...
function accessField(meta, access) {
    ...
    for (const name of known) {
        ...
        box.addEventListener('change', () => {
            ...
            if (!read.length) {
                if (meta.access) {
                    delete meta.access.read;
                    if (!Object.keys(meta.access).length)
                        meta.access = null;
                }
            }
            else {
    ...
    hidden.addEventListener('change', () => {
        if (hidden.checked)
            meta.access = Object.assign({}, meta.access || {}, { hidden: true });
        else if (meta.access) {
            delete meta.access.hidden;
            if (!Object.keys(meta.access).length)
                meta.access = null;
        }
        redraw();
...
function docPicker(label, arr, allDocs, selfId) {
    ...
    select.addEventListener('change', () => {
        if (select.value && !arr.includes(select.value)) {
...
export function openNewDocModal(opts) {
    ...
    for (const s of opts.sources)
        append(sourceSelect, elem('option', { value: s.name }, s.name + '  (' + s.component + ')'));
    ...
    function submit() {
        const id = docId();
        if (!id || !pathInput.value.trim()) {
            error.textContent = 'Enter a path.';
            return;
        }
        if (!/^[\w/-]+$/.test(pathInput.value.trim().replace(/\.md$/i, ''))) {
            error.textContent = 'Path may contain letters, numbers, - _ / only.';
            return;
        }
        if (opts.exists(id)) {
            error.textContent = 'A document with that path already exists.';
            return;
        }
        close();
        opts.onCreate(id);
