...
function fencedRanges(body) {
    ...
    if (open)
        ranges.push([open.from, String(body).length]); // an unterminated fence runs to the end
    return ranges;
}
/**
 ...
 * @returns {string}
 */
function pick(rec, names) { for (const n of names)
    if (rec[n] !== undefined && rec[n] !== '')
        return rec[n]; return ''; }
 ...
 */
function resolveIn(idx, cands) { for (const c of cands)
    if (idx.has(c))
...
export function resolveReqRef(raw, ctx) {
    ...
    if (ctx.group)
        cands.push('R_' + ctx.component + '_' + ctx.group + '_' + raw);
...
export function extractGroups(body, doc, component) {
    ...
    while ((m = RE.exec(body)) !== null) {
        if (inFence(ranges, m.index))
            continue; // literal example in a code fence, not a real block
        let meta = null;
        try {
            meta = JSON.parse(m[1]);
        }
...
function extractReqGroup(meta, table, doc, component) {
    ...
    let error = null;
    if (!component)
        error = "source '" + doc.source + "' has no 'component' id in the server config";
    else if (!meta)
        error = 'requirement-group metadata is not valid JSON';
    else if (!group)
        error = 'requirement-group name is missing';
    /** @type {ReqGroupBlock} */
...
function extractTestCase(meta, table, doc, component) {
    ...
    if (!component)
        ...
        error = 'test-case metadata is not valid JSON';
    else if (!key)
        error = 'test-case key ("test") is missing';
...
export function preprocessRequirements(body, docId) {
    ...
    return src.replace(metaBlockRe(), (whole, _json, _inner, offset) => {
        if (inFence(ranges, offset))
            return whole; // leave a literal example untouched
        const key = docId + '::' + (n++);
        return '\n```reqgroup\n' + key + '\n```\n';
    });
}
