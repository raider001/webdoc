// doclinks.ts - in-body markdown links for the map's "Page link" layer.
// ---------------------------------------------------------------------------
...
export function resolveResourceUrl(baseId, src) {
    const s = String(src || '');
    if (!s || HAS_SCHEME.test(s) || s.startsWith('//') || s.startsWith('/'))
        return null;
...
export function resolveDocId(path, baseId, ids) {
    ...
    const match = (c) => {
        ...
        for (const id of ids.keys())
            if (id.toLowerCase() === lc)
                return id; // case-insensitive
        return null;
    };
    if (baseId) {
        const hit = match(joinDocPath(baseId, path));
        if (hit)
            return hit;
    } // relative to the current doc
    const asIs = match(path);
    if (asIs)
        return asIs; // already a full doc id
    const lower = path.replace(/^\.?\//, '').toLowerCase(); // authored without the source segment
    for (const id of ids.keys()) {
        const i = id.indexOf('/');
        if (i >= 0 && id.slice(i + 1).toLowerCase() === lower)
            return id;
    }
    return null;
}
...
export function documentLinks(docs, traceEdges) {
    ...
    for (const d of docs || []) {
        for (const p of d.assumes || [])
            exclude.add(pairKey(d.id, p));
        for (const s of d.next || [])
            exclude.add(pairKey(d.id, s));
    }
    for (const te of traceEdges || [])
        exclude.add(pairKey(te.from, te.to));
    ...
    for (const d of docs || []) {
        ...
        while ((m = linkRe.exec(body)) !== null) {
            ...
            if (!target || target.charAt(0) === '#')
                continue; // empty or in-page anchor
            ...
            if (resolved) {
                if (resolved === d.id)
                    continue; // self-link
                if (exclude.has(pairKey(d.id, resolved)))
                ...
                if (!extById.has(extId))
                    extById.set(extId, { id: extId, url: target });
                const ek = d.id + ' ' + extId;
                if (seenEdge.has(ek))
                    continue;
                seenEdge.add(ek);
