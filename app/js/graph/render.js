...
function covLabel(st, kind) {
    if (kind === 'test')
        return st.status === 'pass' ? 'Pass' : st.status === 'fail' ? 'Fail' : st.status === 'partial' ? 'Partial' : 'Untested';
...
export function computeNodeSize(g) {
    ...
    for (const meta of g.model.nodes.values())
        titleW = Math.max(titleW, ctx.measureText(meta.title || '').width);
    const W = clamp(Math.ceil(titleW) + 28, LO.nodeW, g.opts.maxNodeW || 360);
    let hasStatus = false, hasDesc = false;
    for (const meta of g.model.nodes.values()) {
        if (g.statusOf(meta.id))
            hasStatus = true;
        if (meta.desc || meta.missing)
            hasDesc = true;
    ...
    if (hasStatus)
        H += 16; // coverage/status line
    if (hasDesc)
        H += 30; // description (up to two lines) or the "missing" tag
    ...
    if (g.externalNodes.length) {
        ...
        const hits = (x, y) => {
            for (const [, n] of layout.nodes)
                if (x < n.x + n.w + PAD && x + EXT_W + PAD > n.x && y < n.y + n.h + PAD && y + EXT_H + PAD > n.y)
                    return true;
            for (const [, p] of extPos)
                if (x < p.x + p.w + PAD && x + EXT_W + PAD > p.x && y < p.y + p.h + PAD && y + EXT_H + PAD > p.y)
                    return true;
            return false;
        };
        ...
        for (const en of g.externalNodes) {
            let src = null;
            for (const s of (srcOf.get(en.id) || [])) {
                const p = layout.nodes.get(s);
                if (p) {
                    src = p;
                    break;
                }
...
export function renderScene(g) {
    ...
    g.requestDraw = function () {
        ...
        g.rafId = requestAnimationFrame(function () {
            ...
            if (g.tween)
                stepTween(g);
            draw(g);
            if (g.tween || (g.flashUntil && performance.now() < g.flashUntil))
                g.requestDraw();
            else if (g.flashUntil && performance.now() >= g.flashUntil) {
                g.flashId = null;
                g.flashUntil = 0;
                draw(g);
            }
    ...
    g.drawStop = function () { g.running = false; if (g.rafId)
        cancelAnimationFrame(g.rafId); g.rafPending = false; };
    ...
    try {
        g.themeObs.observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme'] });
    }
    catch (e) { }
    ...
    g.nodeAtWorld = function (wx, wy) {
        ...
        for (let i = ids.length - 1; i >= 0; i--) {
            ...
            if (n && wx >= n.x && wx <= n.x + n.w && wy >= n.y && wy <= n.y + n.h) {
                ...
                if (meta && meta.missing && !g.vis.missing)
                    continue;
                return ids[i];
            }
        }
    ...
    g.edgeAtWorld = function (wx, wy, tol) {
        ...
        for (const e of g.edgeItems) {
            if (!g.vis[e.cat])
                continue;
            const a = e.aabb;
            if (wx < a[0] - bestD || wx > a[2] + bestD || wy < a[1] - bestD || wy > a[3] + bestD)
                continue;
            const pts = e.pts;
...
function buildGrid(g) {
    ...
    for (const [id, n] of g.layout.nodes)
        add(id, n.x, n.y, n.w, n.h);
    if (g.extPos)
        for (const [id, p] of g.extPos)
            add(id, p.x, p.y, p.w, p.h);
...
function buildEdges(g) {
    ...
    const push = (from, to, type, cat, pts, head) => {
        ...
        let af = byNode.get(from);
        if (!af)
            byNode.set(from, af = []);
        af.push(idx);
        let at = byNode.get(to);
        if (!at)
            byNode.set(to, at = []);
        at.push(idx);
    };
    for (const e of g.layout.edges)
        push(e.from, e.to, e.type, e.type, e.points, e.head);
...
export function startTween(g, from) {
    ...
    if (g.extPos)
        g.extPos.forEach((p, id) => { const o = from.get(id); if (o && (Math.abs(o.x - p.x) > 0.5 || Math.abs(o.y - p.y) > 0.5))
            items.push({ id: id, ox: o.x, oy: o.y, nx: p.x, ny: p.y }); });
    if (!items.length)
        return;
...
function stepTween(g) {
    ...
    for (const it of tw.items)
        g.tweenOffset.set(it.id, { dx: (it.ox - it.nx) * (1 - e), dy: (it.oy - it.ny) * (1 - e) });
    if (t >= 1) {
        g.tween = null;
        g.tweenOffset = null;
    }
...
function draw(g) {
    ...
    if (!g.layout.nodes.size && !(g.extPos && g.extPos.size))
        return;
    ...
    const posOf = (id) => { const n = g.nodePos(id); if (!n)
        ...
        const d = off.get(id);
        if (d)
            return { x: n.x + d.dx, y: n.y + d.dy, w: n.w, h: n.h };
    } return n; };
    const vis = g.vis;
    ...
    for (let cxi = cx0; cxi <= cx1; cxi++)
        for (let cyi = cy0; cyi <= cy1; cyi++) {
            ...
            for (const id of ids) {
                ...
                if (meta && meta.missing && !vis.missing)
                    continue;
                if (isExt && !vis.pagelink)
                    continue;
                const st = g.statusOf(id);
                if (st && g.hiddenStatuses.has(st.status))
                    continue;
                const n = posOf(id);
                if (!n)
                    continue;
                const sx = px(n.x), sy = py(n.y), sw = n.w * k, sh = n.h * k;
                if (sx > cw || sy > ch || sx + sw < 0 || sy + sh < 0)
                    continue;
    ...
    if (tier > 0 || g.focusId) {
        ...
        for (const vn of viz) {
            ...
            for (const ei of eis) {
                ...
                if (!vis[e.cat])
                    continue;
                if (g.focusId && e.from !== g.focusId && e.to !== g.focusId)
                    continue;
                if (tier === 0 && !(g.focusId || e.from === g.currentId || e.to === g.currentId))
                    continue;
                if ((vis.missing === false) && touchesMissing(g, e))
                    continue;
                if (g.hiddenStatuses.size) {
                    const sf = g.statusOf(e.from), st2 = g.statusOf(e.to);
                    if ((sf && g.hiddenStatuses.has(sf.status)) || (st2 && g.hiddenStatuses.has(st2.status)))
                        continue;
                }
                const pts = e.pts;
                ...
                ctx.stroke();
                if (tier === 2)
                    drawArrow(ctx, pts, e, k, tx, ty, catColor(C, e.cat));
            }
        }
        ctx.setLineDash([]);
    }
...
function drawNode(g, ctx, id, meta, isExt, st, sx, sy, sw, sh, tier) {
    ...
    if (isExt) {
        ...
        ctx.fillStyle = mix(statusColor(C, st.status), C.surface, missing ? 0 : 0.13);
    }
    else if (isCurrent) {
    ...
    if (hasDesc && tier === 2) {
        ...
        for (const ln of dLines) {
            ctx.fillText(ln, sx + pad, cy + 6 * g.k);
            cy += 13 * g.k;
        }
    }
    if (fade)
...
function dimmed(g, meta) {
    ...
    if (!groups.length || !g.hiddenGroups || !g.hiddenGroups.size)
        return false;
...
function drawAccess(g, ctx, meta, sx, sy, sw, sh, tier) {
    const groups = (meta && meta.groups) || [];
    if (!groups.length && !(meta && meta.locked))
        return;
...
function hex(s) {
    ...
    if (!m) {
        const rm = /rgba?\(([^)]+)\)/.exec(s);
        if (rm) {
            const p = rm[1].split(',').map(n => parseFloat(n));
            return [p[0], p[1], p[2]];
        }
        return null;
    }
