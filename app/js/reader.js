...
function resolveImages(article, baseId) {
    article.querySelectorAll('img[src]').forEach(img => {
        ...
        if (!src)
            return; // the img[src] selector above is what guarantees this
...
function setupScrollSpy(article, tocList) {
    ...
    tocLinks.forEach(a => { const target = a.dataset.target; if (target !== undefined)
        links.set(target, a); });
...
export function setupDocSearch() {
    const input = mustEl('docSearch');
    input.addEventListener('input', () => {
        const article = mustEl('content').querySelector('.doc');
        if (!article)
            return;
...
function highlight(root, q) {
    ...
    for (const text of targets) {
        ...
        let lower = s.toLowerCase(), i = 0, idx;
