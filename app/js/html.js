// html.js - tagged-template DOM builder for STATIC shape: report tables, cards,
// list items - anything that reads better as markup than as nested elem() calls.
//
// Child positions only: ${value} fills a child slot the same way dom.js's
...
export function fillSlots(root, values) {
    ...
    while ((node = /** @type {Comment} */ (walker.nextNode())))
        if (node.data === 'slot')
            slots.push(node);
