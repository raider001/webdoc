...
import type { TestStep } from './requirements.js';
import type { CoverageStatus } from './coverage.js';
import type { TestReportData } from './coverage-view.js';

/**
 * One test's row of the report's evidence appendix, as coverage-view.js
 * assembles it. Named here (off TestReportData rather than redeclared) only so
 * the lookup map below can be given its key/value types.
 */
 ...
 * @returns HTML
 */
function stepsCell(steps: TestStep[]): string {
...
const LABEL: Record<string, string> = { pass: 'Pass', fail: 'Fail', partial: 'Partial', untested: 'Untested' };
...
function statusOf(map: Map<string, CoverageStatus> | Record<string, CoverageStatus>, id: string): CoverageStatus | undefined | null {
  if (!map) return null;
 ...
 * @returns HTML
 */
function refList(ids: string[], kind: string): string {
 ...
 */
export function generateReportHtml(data: TestReportData): string {
  const reqs = data.requirements || [];
  const tests = data.tests || [];
  const reqStatus = data.reqStatus, testStatus = data.testStatus;
  const detailById = new Map<string, ReportDetail>((data.detail || []).map(d => [d.id, d]));

  // ---- summary counts ----
  const rc: Record<string, number> = { pass: 0, partial: 0, fail: 0, untested: 0 };
  reqs.forEach(r => { const cs: Partial<CoverageStatus> = statusOf(reqStatus, r.id) || {}; const s = cs.status || 'untested'; rc[s] = (rc[s] || 0) + 1; });
  const tc: Record<string, number> = { pass: 0, fail: 0, untested: 0 };
  tests.forEach(t => { const cs: Partial<CoverageStatus> = statusOf(testStatus, t.id) || {}; const s = cs.status || 'untested'; tc[s] = (tc[s] || 0) + 1; });
   ...
   * @returns HTML
   */
  function bar(counts: Record<string, number>, total: number, order: string[]): string {
  ...
  const reqRows = reqs.map(r => {
    const s: Partial<CoverageStatus> = (statusOf(reqStatus, r.id) || {});
  ...
  const testRows = tests.map(t => {
    const s: Partial<CoverageStatus> = (statusOf(testStatus, t.id) || {});
