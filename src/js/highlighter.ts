...
const LANGS: Record<string, HighlightTokenizer> = {
 ...
 */
function langOf(code: HTMLElement): string | null {
 ...
 */
function rebuild(code: HTMLElement, toks: HighlightToken[]): void {
  code.textContent = '';
  for (const t of toks) {
    if (!t.type) {
