 ...
 */
export async function buildRequirementIndex(_docs: Doc[] | null, sources: SourceConfig[]): Promise<void> {
  index.clear();
  testIndex.clear();
  groupsByDoc.clear();
  ...
  let data: CoverageIndexResponse | null = null;
  try {
 ...
 */
export function prepareDocGroups(body: string, docId: string, source: string): ReqOrTestBlock[] {
 ...
 */
export function requirementList(): RequirementEntry[] {
  return [...index.values()]
    .map(r => ({
 ...
 */
export function testList(): TestCaseEntry[] {
...
function reveal(prefix: string, id: string): void {
  if (!id) return;
...
export function revealRequirement(reqId: string): void { reveal('req-', reqId); }
export function revealTest(testId: string): void { reveal('test-', testId); }
 ...
 */
export function routeParams(query: string): { req: string | null, test: string | null } {
 ...
 */
export function requirementTraceEdges(): DocEdgeRef[] {
  const seen = new Set<string>();
  const edges: DocEdgeRef[] = [];
