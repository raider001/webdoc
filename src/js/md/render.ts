 ...
 */
export function renderTree(doc: MdBlockNode, refs: Record<string, RefDefinition>): string {
  return renderChildren(doc, refs, false);
}
/**
 * @param tight whether the enclosing list is tight (suppresses paragraph <p> wrapping)
 */
function renderChildren(block: MdBlockNode, refs: Record<string, RefDefinition>, tight: boolean): string {
  let out = '';
...
function renderBlock(b: MdBlockNode, refs: Record<string, RefDefinition>, tight: boolean): string {
  switch (b.type) {
    case 'paragraph': {
