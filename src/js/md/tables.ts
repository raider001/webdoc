...
import type { MdBlockNode, MdTableBlock, TableAlign } from './blocks.js';
 ...
 */
interface TableBuildInfo {
  /** index into `lines` of the header row */
  headerIndex: number;
  /** index into `lines` of the delimiter row (headerIndex + 1) */
  delimIndex: number;
 ...
 */
function tryBuildTable(lines: string[]): TableBuildInfo | null {
 ...
 */
export function extractTables(block: MdBlockNode): void {
 ...
 */
export function renderTable(b: MdTableBlock, refs: Record<string, RefDefinition>): string {
  const cols = b.aligns.length;
  /** @param a this column's alignment, null for the default */
  const attr = (a: TableAlign): string => a ? ' align="' + a + '"' : '';
  /** @param text raw cell markdown */
  const cell = (tag: string, text: string, a: TableAlign): string => '<' + tag + attr(a) + '>' + parseInlines(text || '', refs) + '</' + tag + '>\n';
  let out = '<table>\n<thead>\n<tr>\n';
