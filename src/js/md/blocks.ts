 ...
 */
export function makeBlock<K extends keyof MdBlockTypeMap>(
  type: K,
  extra?: Partial<MdBlockTypeMap[K]>
): MdBlockTypeMap[K] {
  const b = { type: type, children: [], open: true, lines: [], lastLineBlank: false } as unknown as MdBlockTypeMap[K];
  ...
  if (extra) {
    ...
    for (const k in src) bag[k] = src[k];
  }
  return b;
}
...
export function parseDocument(src: string): ParsedDocument {
  ...
  const refs: Record<string, RefDefinition> = Object.create(null);
  const path: MdBlockNode[] = [doc]; // open blocks, document -> tip
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (matched === path.length - 1 && path[path.length - 1].type === 'paragraph' &&
      ...
      const para = path[path.length - 1] as MdParagraphBlock;
...
function last(b: MdBlockNode): MdBlockNode | undefined { return b.children[b.children.length - 1]; }

function canContain(b: MdBlockNode, childType: string): boolean {
 ...
 */
function startsNewBlock(rest: string, _leaf: MdBlockNode, interrupting: boolean): boolean {
 ...
 * @param path open blocks, document -> tip
 */
function continuesOpenList(rest: string, path: MdBlockNode[]): boolean {
 ...
 */
function isRefOnly(para: MdBlockNode): boolean {
