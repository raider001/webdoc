 ...
 */
export function scanDest(text: string, i: number): LinkDestScan | null {
  if (text[i] === '<') {
    let j = i + 1, dest = '';
    while (j < text.length) {
      const c = text[j];
 ...
 * @param i index to start scanning from
 */
export function scanTitle(text: string, i: number): LinkTitleScan | null {
  const open = text[i];
 ...
 */
export function normLabel(s: string): string { return s.replace(/[ \t\r\n]+/g, ' ').trim().toLowerCase().toUpperCase().toLowerCase(); }
 ...
 */
export function scanBracketLabel(s: string, i: number): BracketLabelScan | null {
  if (s[i] !== '[') return null;
  let j = i + 1, label = '';
  while (j < s.length) {
    const c = s[j];
