 ...
 */
export function joinDocPath(baseId: string, rel: string): string {
 ...
 */
export function resolveResourceUrl(baseId: string, src: string): string | null {
  const s = String(src || '');
 ...
 */
export function resolveDocId(path: string, baseId: string | null, ids: Set<string> | Map<string, unknown>): string | null {
  if (!path || !ids) return null;
  const match = (c: string): string | null => {
    if (!c) return null;
 ...
 */
export function documentLinks(docs: Doc[], traceEdges: DocEdgeRef[]): DocLinksResult {
  ...
  const pageLinks: DocEdgeRef[] = [];
  const extById = new Map<string, ExternalLinkNode>();
