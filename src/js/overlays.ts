...
let mapParts: OverlayHost | null = null;
let covParts: OverlayHost | null = null;

function makeHost(id: string, cls: string): OverlayHost {
 ...
 */
export function ensureOverlayHosts(): void {
...
function need(parts: OverlayHost | null, which: string): OverlayHost {
  if (!parts) throw new Error('overlays: ' + which + ' host requested before ensureOverlayHosts() ran');
  return parts;
}

export function mapOverlay(): OverlayHost { return need(mapParts, 'map'); }
export function coverageOverlay(): OverlayHost { return need(covParts, 'coverage'); }
...
export function coverageOpen(): boolean {
 ...
 */
export async function requestMapRebuild(): Promise<void> {
  if (!mapOpen()) return;
