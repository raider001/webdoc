...
// ---------------------------------------------------------------------------
import {
  lineLen, blockLen, quoteLen, atLineStart, prevIsBoundary, nextIsParen,
  lineStartIdx, RE_ID, RE_NUM_GEN, RE_NUM_PY, RE_NUM_JAVA, Lexer
} from './lexer.js';
import type { HighlightToken } from './lexer.js';
 ...
 */
function pyStringAt(src: string, i: number): number {
...
export function pythonTokens(src: string): HighlightToken[] {
 ...
 */
function javaStringAt(src: string, i: number): number {
  if (src.startsWith('"""', i)) {
    let j = i + 3;
    while (j < src.length) {
...
export function javaTokens(src: string): HighlightToken[] {
  const lx = new Lexer(src);
  while (!lx.eof()) {
    const c = lx.peek();
 ...
 */
function shParenSub(src: string, i: number): number {
  let depth = 0;
 ...
 */
function shBraceVar(src: string, i: number): number {
  let j = i + 2;
 ...
 */
function shBacktick(src: string, i: number): number {
  let j = i + 1;
  while (j < src.length && src[j] !== '`') j++;
  return (j < src.length ? j + 1 : j) - i;
}

export function shellTokens(src: string): HighlightToken[] {
  const lx = new Lexer(src);
  while (!lx.eof()) {
    const c = lx.peek();
 ...
 */
function mkInRecipe(src: string, i: number): boolean { return src[lineStartIdx(src, i)] === '\t'; }

export function makefileTokens(src: string): HighlightToken[] {
  const lx = new Lexer(src);
  while (!lx.eof()) {
    const c = lx.peek();
    if (c === '#') { lx.push('comment', lineLen(src, lx.pos)); continue; }
...
export function robotTokens(src: string): HighlightToken[] {
  const lx = new Lexer(src);
  while (!lx.eof()) {
    const c = lx.peek();
...
export function genericTokens(src: string): HighlightToken[] {
  const lx = new Lexer(src);
  while (!lx.eof()) {
    const c = lx.peek();
    if (c === '/' && lx.peek(1) === '/') { lx.push('comment', lineLen(src, lx.pos)); continue; }
