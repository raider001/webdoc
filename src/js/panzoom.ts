 ...
 */
export function createPanZoom(content: HTMLElement | SVGElement, opts: PanZoomOptions = {}): HTMLElement {
  ...
  viewport.setAttribute('aria-label',
    'Pan and zoom diagram. Drag to pan; use the buttons, or the scroll wheel once focused, to zoom.');
  ...
  const clamp = (v: number) => Math.max(minK, Math.min(maxK, v));
   ...
   * scale `k`.
   */
  function naturalSize(): NaturalSize {
    const svg = (content.tagName && content.tagName.toLowerCase() === 'svg')
      ? content as SVGSVGElement : content.querySelector('svg');
   ...
   */
  function zoomAt(factor: number, ax: number, ay: number): void {
   ...
   */
  const button = (icon: Element, title: string, fn: () => void): HTMLButtonElement => {
  ...
  viewport.addEventListener('pointerdown', (e) => {
    if ((e.target as Element).closest('.pz-controls')) return;
  ...
  viewport.addEventListener('wheel', (e) => {
    if (document.activeElement !== viewport) return; // let the page scroll normally
    e.preventDefault();
  ...
  viewport.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      ...
      case '+': case '=': zoomAt(1.2, midX(), midY()); e.preventDefault(); return;
      case '-': case '_': zoomAt(1 / 1.2, midX(), midY()); e.preventDefault(); return;
      case '0': fit(); e.preventDefault(); return;
      default: return;
    }
  ...
  const ro = new ResizeObserver(() => {
    if (viewport.clientWidth > 0 && viewport.clientHeight > 0) { fit(); ro.disconnect(); }
  });
  ro.observe(viewport);
