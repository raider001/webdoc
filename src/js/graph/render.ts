...
import type { GraphContext, GraphBox, GraphEdgeItem } from '../graph.js';
import type { ModelGraphNode, LayoutNode } from './layout.js';
 ...
 */
function covLabel(st: CoverageStatus, kind: string): string {
 ...
 */
export function positionExternal(g: GraphContext): void {
  const extPos = new Map<string, GraphBox>();
  ...
  if (g.externalNodes.length) {
     ...
     */
    const nearestFree = (ax: number, ay: number): { x: number, y: number } => {
 ...
 */
function readColors(el: Element): GraphColors {
...
const CAT_DASH: Record<string, number[]> = { prereq: [], recnext: [6, 5], trace: [2, 3], pagelink: [5, 3] };
/**
 ...
 */
function catColor(colors: GraphColors, cat: string): string { return cat === 'recnext' ? colors.recnext : cat === 'trace' ? colors.trace : cat === 'pagelink' ? colors.pagelink : colors.prereq; }

// Point on node n's border in the direction of (tx,ty) - for straight overlay edges.
function borderPoint(n: GraphBox, tx: number, ty: number): { x: number, y: number } {
 ...
 */
export function renderScene(g: GraphContext): void {
 ...
 * layout is empty.
 */
function firstNode(g: GraphContext): LayoutNode | { w: number, h: number } { for (const n of g.layout.nodes.values()) return n; return { w: LO.nodeW, h: LO.nodeH }; }
 ...
 */
function buildGrid(g: GraphContext): void {
   ...
   */
  const add = (id: string, x: number, y: number, w: number, h: number): void => {
 ...
 */
function buildEdges(g: GraphContext): void {
  const items: GraphEdgeItem[] = [];
  const byNode = new Map<string, number[]>();     // node id -> INDICES into `items`
  /**
   ...
   */
  const push = (from: string, to: string, type: string, cat: string, pts: { x: number, y: number }[], head: string): void => {
 ...
 */
export interface TweenItem { id: string; ox: number; oy: number; nx: number; ny: number }
 ...
 */
export function startTween(g: GraphContext, from: Map<string, { x: number, y: number }>): void {
 ...
 */
function stepTween(g: GraphContext): void {
...
/**
 * One node that survived the viewport cull, with its screen rect already
 * computed - collected in pass A so pass C can draw the nodes after the edges
 * without hit-testing the grid a second time.
 */
 ...
 */
function draw(g: GraphContext): void {
  g.dirty = false;
  ...
  const px = (x: number) => x * k + tx, py = (y: number) => y * k + ty;
  /**
   ...
   */
  const posOf = (id: string): GraphBox | null => { const n = g.nodePos(id); if (!n) return null; if (off) { const d = off.get(id); if (d) return { x: n.x + d.dx, y: n.y + d.dy, w: n.w, h: n.h }; } return n; };
  const vis = g.vis;
  ...
  const occ = tier === 0 ? new Set<string>() : null;
  const viz: VisibleNode[] = [];
  ...
  if (tier > 0 || g.focusId) {
    const se = g.selectedEdge, drawn = new Set<number>();
    for (const vn of viz) {
 ...
 */
function doff(off: Map<string, { dx: number, dy: number }> | null, id: string, ax: 'x' | 'y'): number { if (!off) return 0; const d = off.get(id); return d ? (ax === 'x' ? d.dx : d.dy) : 0; }
/**
 ...
 */
function drawArrow(ctx: CanvasRenderingContext2D, pts: { x: number, y: number }[], e: { type: string, head: string }, k: number, tx: number, ty: number, color: string): void {
 ...
 */
function statusColor(C: GraphColors, s: string): string { return C.st[s] || C.muted; }
/**
...
function mix(a: string, b: string, w: number): string {
  ...
  if (!pa || !pb) return a;
  const c = (i: number) => Math.round(pa[i] * w + pb[i] * (1 - w));
