 ...
 */
function toWorld(g: GraphContext, clientX: number, clientY: number): { wx: number, wy: number } {
 ...
 */
export function wireInteractions(g: GraphContext): void {
  const listeners: ListenerReg[] = [];
  /**
  ...
  let downExtUrl: string | null = null;
  let downEdge: { from: string, to: string, type: string } | null = null;
  ...
  if (mini) on(mini, 'click', function (e) {
    try {
