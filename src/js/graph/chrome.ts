 ...
 */
export function attachEditState(g: GraphContext): void {
  /**
  ...
  g.setConnector = function (type) {
    if (type !== 'prereq' && type !== 'recnext') return;   // the only two the engine can draw
    if (g.activeConnector === type) return;
  ...
  g.clearPending = function () {
    if (g.pendingSource === null) return;                  // no repaint, no emit, for a no-op
    g.pendingSource = null; g.requestDraw(); g.emitChange();
  };
  g.clearSelectedEdge = function () {
    if (g.selectedEdge === null) return;
    g.selectedEdge = null; g.requestDraw(); g.emitChange();
  };
