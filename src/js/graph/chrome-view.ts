...
import type { GraphOptions, GraphController, GraphChangeEvent } from '../graph.js';
 ...
 */
export function mountGraphChrome(container: HTMLElement, options?: GraphOptions): GraphChromeHandle {
  const opts = options || {};
  ...
  const owned: Element[] = [];              // everything this module put in the container, for destroy()
  const cleanups: (() => void)[] = [];      // document-level listeners the dropdown may have left open
   ...
   */
  let ctl: GraphController | null = null;
  let unsubscribe: (() => void) | null = null;

  const own = <T extends Element>(node: T): T => { frag.appendChild(node); owned.push(node); return node; };

  /**
   * @param icon - an inline SVG built by icons.js
   * @param aria - doubles as the tooltip, so the label is never mouse-only
   */
  const ctrlBtn = (icon: Element, aria: string): HTMLButtonElement => elem('button', { type: 'button', class: 'graph-ctrl-btn', 'aria-label': aria, title: aria }, icon);

  // ---- Zoom controls ----
  const btnIn = ctrlBtn(plusIcon(), 'Zoom in');
  const btnOut = ctrlBtn(minusIcon(), 'Zoom out');
  const btnFit = ctrlBtn(fitIcon(), 'Fit to view');
  btnIn.addEventListener('click', () => { if (ctl) ctl.zoomBy(1.25); });
  btnOut.addEventListener('click', () => { if (ctl) ctl.zoomBy(1 / 1.25); });
  btnFit.addEventListener('click', () => { if (ctl) ctl.fit(); });
  ...
  searchInput.addEventListener('input', () => { if (ctl) ctl.search(searchInput.value); });
  searchInput.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter') return;
    e.preventDefault();
    if (ctl) ctl.search(searchInput.value);
  });
  ...
  const legendBtns: Record<string, HTMLButtonElement> = {};
  /**
   ...
   */
  function legendToggle(kind: string, label: string): HTMLElement {
    const b = elem('button', { type: 'button', class: 'graph-legend-item', 'data-kind': kind, 'aria-pressed': 'true', title: 'Toggle ' + label },
      elem('span', 'graph-legend-swatch ' + kind), label);
    b.addEventListener('click', function () {
      if (!ctl) return;
      const s = ctl.getEditState();
      if (s.editMode && (kind === 'prereq' || kind === 'recnext')) { ctl.setConnector(kind); return; }
  ...
  if (!opts.hideLegend) {
    ...
    if (modes && modes.length > 1) {
      const dd = buildMapModeDropdown(modes, opts.mapMode || modes[0].value, (m) => { if (ctl) ctl.setMapMode(m); });
    ...
    append(legend,
      ...
      (opts.traceEdges && opts.traceEdges.length) ? legendToggle('trace', 'Requirement trace') : null,
      (opts.pageLinks && opts.pageLinks.length) ? legendToggle('pagelink', 'Page link') : null,
  ...
  if (accessGroups.length && !opts.hideLegend) {
    ...
    append(panel, elem('p', 'graph-groups-note',
      'A locked page shows a padlock: it exists, but your account cannot open it.'));
    own(panel);
  }
  ...
  if ((opts.onConnect || opts.onDisconnect) && !opts.hideLegend) {
    ...
    editBtn.addEventListener('click', () => { if (ctl) ctl.setEditMode(!ctl.getEditState().editMode); });
    hintEl = elem('div', { class: 'graph-edit-hint', hidden: true });
    own(editBtn); own(hintEl);
  }
  ...
  if (opts.onFocusToggle) {
    ...
    focusBtn.addEventListener('click', () => { if (opts.onFocusToggle) opts.onFocusToggle(); });
    own(focusBtn);
  }
   ...
   */
  function render(s: GraphChangeEvent): void {
  ...
  return {
    ...
    connect: function (controller) {
      ...
      if (controller.getNodePositions().size === 0) own(elem('div', 'graph-empty', 'No documents to map.'));
    ...
    destroy: function () {
      ...
      for (const fn of cleanups) { try { fn(); } catch (err) { /* teardown is best-effort */ } }
      cleanups.length = 0;
      for (const node of owned) { if (node.parentNode) node.parentNode.removeChild(node); }
      owned.length = 0;
...
function buildMapModeDropdown(modes: { value: string, label: string, swatch: string }[], current: string, onPick: (mode: string) => void): { el: HTMLElement, close: () => void } {
  /** @param cls - edge category of the map mode; blank/absent means the 'all' modes swatch */
  const swatch = (cls: string) => elem('span', 'graph-legend-swatch ' + (cls || 'all'));
  ...
  const btnLabel = elem('span', 'graph-mapmode-label',
    modes.map(m => elem('span', { class: m.value === current ? 'is-cur' : null }, m.label)));
