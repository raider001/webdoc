 ...
 * and `html` are what is actually compared.
 */
interface SpecCase {
  markdown: string;
  html: string;
...
function textCell(label: string, value: string, cls?: string) {
...
async function run() {
  ...
  const order: string[] = []; // preserve first-seen section order (== spec order)
  let pass = 0;
  ...
  for (const name of order) {
    ...
    const td = (txt: string, cls?: string) => { const d = document.createElement('td'); if (cls) d.className = cls; d.textContent = txt; return d; };
