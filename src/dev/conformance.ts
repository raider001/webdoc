 ...
 *
 * Every field is optional except by convention: the first entry is a `_note`
 * header carrying only prose, which is exactly why the filter below keeps only
 * entries whose `markdown` is a string rather than trusting the array's shape.
 */
...
async function run() {
  ...
  for (const c of cases) {
    ...
    sec.className = 'sec'; sec.textContent = c.section || '(section)';
