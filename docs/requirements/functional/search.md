...
search box can match across the whole set and jump straight to a document or a
heading within it. Search is deliberately scoped to titles and headings rather
than full body text, keeping it light and fast. Both rows trace up to `sys_3`
(let a reader navigate the whole document set) and compose to `WD_search_{no}`.
